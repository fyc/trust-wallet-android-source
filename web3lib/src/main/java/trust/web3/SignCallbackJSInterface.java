...
import trust.web3.utils.EthRequestManager;
...
public class SignCallbackJSInterface {
    ...
    AtomicInteger requestTimes = new AtomicInteger(3);
    ...
    public void signTransaction2(
        ...
        this.from = from;
        requestTimes.set(3);
        EthRequestManager.eth_gasPrice(new EthRequestManager.EthRequestCall() {
            @Override
        ...
        EthRequestManager.eth_estimateGas(from, recipient, value, payload, new EthRequestManager.EthRequestCall() {
            @Override
            public void success(String result) throws IOException {
                gasLimit = result;
                doSignTransaction2();
        ...
        EthRequestManager.eth_getTransactionCount(from, new EthRequestManager.EthRequestCall() {
            @Override
            public void success(String result) throws IOException {
                nonce = result;
                doSignTransaction2();
    ...
    private void doSignTransaction2() {
        if (requestTimes.decrementAndGet() == 0) {
